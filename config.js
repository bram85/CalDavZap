...
/*
   ...
   * hrefLabel sets the server name in the resource header - useful if you want
     to see a custom resource header above the collections; you can use the
     following variables:
     ...
     * %U = logged user name,
     * %u = logged user name without the @domain.com part (if present); if
            undefined, empty or or null then '%d/%p [%u]' is used.

   * forceReadOnly sets the resource or list of collections as "read-only" - if
     true then the whole resource will be "read-only"; if an array of URL encoded
     collections or regexes (for example: ['/caldav.php/user/calendar/',
     '/caldav.php/user%40domain.com/calendar/', new
     RegExp('^/caldav.php/user/calendar[0-9]/$', 'i')]) then specified
     collections will be marked as "read-only"; if null (default), unset or
     unknown then server detected privileges are used

   * timeOut sets the timeout for jQuery .ajax call (in milliseconds)

   * lockTimeOut sets the LOCK Timeout value if resource locking is used (in
     milliseconds)

   * showHeader shows (true) or hides (false) the resource header in the
     interface

   * settingsAccount sets the account where client properties are saved during
     logout and resource/collection synchronisation (note: set it to true only
     for ONE account)

   * checkContentType enables content-type checking for server response (only
     objects with proper content-type are inserted into interface) - if you
     cannot see data in the interface you may try to disable it (useful if your
     server return wrong value in "propstat/prop/getcontenttype"); if undefined
     content-type checking is enabled

   * delegation sets additional delegated (shared) resources - if true then
     delegation is enabled for all available resources; if false (default) then
     delegation is disabled; if an array of URL encoded resources or regexes
     (for example: ['/caldav.php/user/', '/caldav.php/user%40domain.com/', new
     RegExp('^/caldav.php/a[b-x].+/$', 'i')] then delegation is enabled for all
     specified resources

   * ignoreAlarms defines an array of calendars with disabled alarm - if true
     then alarm is disabled for all collections; if false (default) then alarm
     is enabled for all collections; if an array of URL encoded collections or
     regexes (for example:
     ...
     then alarm is disabled for all specified collections

   * backgroundCalendars defines an array of background calendars - if there is
     at least one event defined for the given day in a background calendar, the
     background color for that day will be pink/light-red; to use this feature
     define an array of URL encoded collections or regexes For example:
   ...
   * crossDomain sets jQuery ajax crossDomain value (use only if you know what
     are you doing!) - by default null = autodetect /detected setting is
     shown in the console/

   * withCredentials sets jQuery's ajax withCredentials value for cross domain
     queries (use only if you know what are you doing!); note: if true,
...
var globalAccountSettings=[
  {
    ...
    hrefLabel: null, forceReadOnly: null,
    showHeader: true,
...
/*
  if href is set, the client authenticates against the href URL (the last
  character in href must be '/') and if the authentication is successful it
  appends the USER + '/' to end of href and sets the userAuth:
  ...
  This option invokes a login screen and disallows access until successfull
  authentication.

  The additionalResources array can contain additional resources (shared
  resources accessible by all users), for example:
  ...
  Lion server example (http + https setup; see misc/readme_lion.txt for server
  setup):
...
/*
  ...
  It is possible to combine this option with the globalAccountSettings
  although it is not recommended this option ivokes a login screen and
  disallows access until the client get correct XML configuration file from
  the server the timeOut sets the timeout for jQuery .ajax call (in
  miliseconds)
*/
// var globalNetworkAccountSettings={href: 'https://www.config-server.com/auth/', timeOut: 30000};

/*
  Default configuration if the auth module is located in the correct
  subdirectory.
*/
/*
var globalNetworkAccountSettings={
  href: location.protocol+'//'+location.hostname+(location.port ? ':'+location.port : '')+location.pathname.replace(RegExp('index\.html$'),'')+'auth/',
  timeOut: 30000};
*/

/*
  Use jQuery .ajax() auth or custom header for HTTP basic auth (default). Set
  this option to true if your server uses digest auth (note: you may experience
  auth popups on some browsers) if undefined (or empty), custom header for HTTP
  basic auth is used.
*/
...
/*
  if defined and not empty then only languages listed here are shown at the login
  screen (for example: ['en_US','sk_SK']), otherwise (default) all languages are
  shown values in the array must refer to an existing localization defined in the
  common.js (see the option above)
*/
var globalInterfaceCustomLanguages=[];

/*
  JavaScript localeCompare() or custom alphabet for data sorting custom alphabet
  is used by default because the JavaScript localeCompare() not support collation
  and often returns "wrong" result
*/
...
/*
  search functionality character equivalence (transformation to ASCII: key =
  regex text, value = result character)
*/
...
/*
  ...
  http://docs.jquery.com/UI/Datepicker/formatDate for valid values) note: date
  format is now predefined for each localization - use this option only if you
  want to use custom date format instead of the predefined one
*/
...
/*
  Enable background sync even if browser window has no focus (if false, sync is
  performed only if browser window/tab is focused)
...
/*
  where to store user settings such as: active view, selected collections, ...
  (we store them into DAV property on the server) note: not all servers support
  storing DAV properties (some servers support only subset /or none/ of these
  URLs)

  if 'principal-URL', '', null or undefined (default) - settings are stored to
  principal-URL
...
/*
  Number of months pre-loaded from past and future in advance (if null then date
  range synchronization is disabled)
...
/*
  Default fullcalendar view option (use 'month', 'multiWeek', 'agendaWeek' or
  'agendaDay')
...
/*
  Set todo calendar to be selected by default after login (URL encoded path to
  the todo calendar, for example: 'USER/todoCalendar/')
...
/*
  Todo calendar collections stored in this array are loaded after login  (if
  empty then all collections are loaded)
...
/*
  Set business hours with 0.5 hour precision - non-business hours will be faded
  out in the calendar interface
...
/*
  ...
  If defined as empty string, no time information will be shown see
  http://arshaw.com/fullcalendar/docs/utilities/formatDate/ for exact
  formatting rules.
*/
...
/*
  Remove non standard timezone names from events and todos on save action (e.g.
...
/*
  ...
  start day of the week is Monday, despite what day is chosen in the settings.
  Set this variable to true to use the same approach, ensuring compatible event
  rendering in special cases
*/
var globalMozillaSupport=false;

/*
  Set what days of the week are considered weekend days; non-weekend days are
  automatically considered to be business days Sunday is 0, Monday is 1, etc.
*/
var globalWeekendDays=[0, 6];

/*
  STRONGLY recommended if you use any Apple clients for todos (has no effect on
  events).

  Accepted values are currently 'iOS6', 'iOS7', true (support of the latest iOS
  version - 'iOS7') and false.
  ...
  If enabled:
   ...
   - When a single instance of repeating todo is edited, it becomes an
     ...
     todo
   - Capabilities of repeating todos are limited - only the first instance is
     ever visible
   - Support for todo DTSTART attribute is disabled
   - Support for todo STATUS attribute other than COMPLETED and NEEDS-ACTION is
     disabled
...
/*
  Array of subscribed (read-only) calendars; each calendar is identified by an
  URL address (for example: http://something.com/calendar.ics)
